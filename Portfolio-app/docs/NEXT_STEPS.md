...
### Sprint 1: Transaction Ledger (거래 원장) 🔴 최우선

**목표:** 거래 입력 및 포지션 계산 기반 구축

#### Backend
1. **Transaction Service 구현**
   - TransactionService.createTransaction()
   - TransactionLeg 검증 로직 (복식부기 밸런싱)
   - Transaction 조회/수정/취소
...
### Sprint 2: Valuation Engine (평가 엔진) 🔴 최우선

**목표:** 실시간 포트폴리오 평가액 계산

#### Backend
1. **Valuation Service**
   - 포지션별 시장가치 계산 (MV = qty × price)
   - 통화 환산 (FX rate 적용)
   - 총 평가액 계산
   - 손익 계산 (실현/미실현)

2. **Price Service (Mock)**
   - 가격 조회 인터페이스
   - Mock 가격 데이터 반환 (개발용)
   - Redis 캐싱 준비

3. **API 엔드포인트**
   - `GET /api/v1/portfolios/{id}/valuation?mode=REALTIME|EOD`
   - `GET /api/v1/portfolios/{id}/positions` - 포지션 목록

#### Frontend
4. **포트폴리오 상세 페이지 데이터 연동**
   - Positions 탭에 실제 보유 종목 표시
   - 평가액, 손익, 비중 표시
   - 실시간 업데이트 (폴링 방식)
...
**예상 기간:** 3-4일  
**우선순위:** 🔴 Critical  
**의존성:** Transaction Ledger 완료 후

---

### Sprint 3: Price Data Integration (가격 데이터 연동) 🟡 중요

**목표:** 실제 시세 데이터 연동

#### Backend
1. **외부 API 연동**
   - Alpha Vantage / Yahoo Finance / KRX API
   - HTTP Client 설정 (WebClient)
...
**예상 기간:** 2-3일  
**우선순위:** 🟡 High  
**의존성:** Valuation Engine 완료 후

---

### Sprint 4: Performance Analytics (성과 분석) 🟡 중요
...
1. **Performance Service**
   ...
   - 기간별 수익률 시계열
   - 리스크 지표 (Volatility, MDD, Sharpe)

2. **API 엔드포인트**
   - `GET /api/v1/portfolios/{id}/performance?from={date}&to={date}&metric=TWR`
   - `POST /api/v1/compare/performance` - 포트폴리오 비교

#### Frontend
3. **Performance 탭 구현**
   - ECharts 라인 차트
   - 기간 선택 (1M/3M/YTD/1Y/전체)
   - 수익률 지표 카드
...
**예상 기간:** 3-4일  
**우선순위:** 🟡 High  
**의존성:** Valuation + Price Data

---

### Sprint 5: Backtesting Engine (백테스트) 🟢 보통
...
### Week 1
- [x] ~~포트폴리오 관리 완성~~ ✅
- [ ] Transaction Ledger 구현
- [ ] Valuation Engine 기본 구현
- [ ] 테스트 안정화
