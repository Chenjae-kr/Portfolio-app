...
- ✅ 리밸런싱 도구 (현재 vs 목표 비중 + 도넛 차트 + 매매 추천)
...
### Sprint 7: Rebalancing Tools (리밸런싱 도구) ✅ 완료

**목표:** 포트폴리오 리밸런싱 분석 및 매매 추천

**✅ 완료된 항목:**
- [x] RebalanceService (현재 vs 목표 비중 비교, 편차 계산)
- [x] 최소 매매 추천 (0.5% 이상 편차 시)
- [x] 거래 비용 추정 (0.1% 수수료율)
- [x] 리밸런싱 상태 판단 (good/warning/danger)
- [x] RebalanceController (GET /v1/portfolios/{id}/rebalance)
- [x] RebalancePanel.vue (ECharts 도넛 차트: 현재 vs 목표 배분)
- [x] 비중 비교 테이블 (종목별 현재/목표/편차/금액)
- [x] 매매 추천 테이블 (BUY/SELL + 거래금액 + 예상 수수료)
- [x] PortfolioDetailView에 Rebalance 탭 통합
- [x] i18n 번역 (한국어/영어 20+ 키)

**🚧 남은 항목:**
- [ ] 현금 제약 조건 반영 (가용 현금 내 매매)
- [ ] 세금 최적화 (손익 실현 시뮬레이션)
- [ ] 리밸런싱 실행 기능 (추천 → 자동 거래 생성)
- [ ] 리밸런싱 이력 저장
